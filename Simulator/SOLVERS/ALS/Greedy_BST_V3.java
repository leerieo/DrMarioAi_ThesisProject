...
public class Greedy_BST_V3 {
    ...
    private int getMatchingSeqLength(int[][] playfield, int row, int col, int color){
        ...
        for(int i=(row+1); i<Bottle.HEIGHT; i++){
            ...
            if(tile!=Bottle.EMPTY_TILE && (tile&Bottle.COLOR_MASK)!=color){
                return seq_count; 
            }
            //if we encounter an empty tile after the sequence has started 
            if(tile==Bottle.EMPTY_TILE && start_seq){
                return seq_count; 
            }
            // if we encounter a matching item 
            if((tile&Bottle.COLOR_MASK)==color){
    ...
    private int getSecondItemBeneath(int[][] playfield, int row, int col){
        ...
        int first_col = Bottle.EMPTY_ID; 

        for(int i=(row+1); i<Bottle.HEIGHT; i++){
            int tile = playfield[i][col];
            //if we encounter a nonmatching item after finding the first tile 
            if(found_first && tile!=Bottle.EMPTY_TILE && (tile&Bottle.COLOR_MASK)!=first_col){
                return tile; 
            }

            //if we encounter an empty tile after the sequence has started 
            if(tile==Bottle.EMPTY_TILE && found_first){
                first_col = Bottle.EMPTY_ID; // gaps break the sequence 
            }

            // if we encounter the first non empty item 
            if(!found_first && tile!=Bottle.EMPTY_TILE){
                found_first = true; // start the sequence count 
                first_col = tile&Bottle.COLOR_MASK; 
            }
        }

        return Bottle.EMPTY_TILE; 
    }
    ...
    // Returns true if the tile given contains a hanging pill 
    private boolean isHangingPill(int[][] playfield, int row, int col){

        // if starting tile is not a vitamin return false 
        if((playfield[row][col]&Bottle.TILE_ID_MASK)==Bottle.VIRUS || playfield[row][col]==Bottle.EMPTY_TILE){
            return false; 
        }

        for(int i=(row+1); i<Bottle.HEIGHT; i++){
            int tile = playfield[i][col];
            // if there is a gap below a vitamin return true 
            if(tile == Bottle.EMPTY_TILE){
                return true; 
            }
    ...
    private boolean itemsBeneathHaveVirus(int[][] playfield, int row, int col){
        for(int i=(row+1); i<Bottle.HEIGHT; i++){
            int tile = playfield[i][col];
            if((tile&Bottle.TILE_ID_MASK)==Bottle.VIRUS){
                return true; 
    ...
    private double scorePlacementVer(Playfield playfield, int row, int col, int orientation, boolean print){
        ...
        if(orientation%2==0){
            ...
            if(row<Bottle.HEIGHT-1){
                ...
                if((tile_l&Bottle.COLOR_MASK) == color_l){
                    score++; 
                    // if the left column has a virus +1
                    if(itemsBeneathHaveVirus(playfield.playfield,row,col)){
                        score++; 
                    }

                    // if left vitamin is not a hanging pill +0.5 
                    if(!isHangingPill(playfield.playfield, row, col)){
                        score+=0.5; 
                    }

                    // left side is a match 
                    left_match = true; 
                }else if(tile_l!=Bottle.EMPTY_TILE){
                    score-=0.5; 
                ...
                if((tile_r&Bottle.COLOR_MASK) == color_r){
                    score++; 

                    // if the right column has a virus +1
                    if(itemsBeneathHaveVirus(playfield.playfield,row,col+1)){
                        score++; 
                    }

                    // if right vitamin is not a hanging pill +0.5 
                    if(!isHangingPill(playfield.playfield, row, col+1)){
                        score+=0.5; 
                    }

                    //right side match is true 
                    right_match = true; 
                }else if(tile_r!=Bottle.EMPTY_TILE){
                    score-=0.5; 
                    //punish non-matches in columns with viruses 
                    if(itemsBeneathHaveVirus(playfield.playfield,row,col+1)){
                        score--; 
            ...
            if(row<Bottle.HEIGHT-1 && color_b==color_t){
                ...
                if((tile&Bottle.COLOR_MASK)==color_b){
                    ...
                    // if vitamin is not a hanging pill +0.5 
                    if(!isHangingPill(playfield.playfield, row, col)){
                        score+=0.5; 
                    }
                ...
                if(match_len>=3){
                    ...
                    if((second_item&Bottle.COLOR_MASK)==color_t){
                        score++; 
                        if(itemsBeneathHaveVirus(playfield.playfield, row, col)){
                            score++; 
                        }    
            ...
            if(row>1){
                ...
                if((tile_t&Bottle.COLOR_MASK) == color_t){
                    score+=0.5; 
                }
                //bonus points for matching beneath items
                if(row<Bottle.HEIGHT-1){
                    int tile_b = playfield.playfield[row+1][col];
                    if((tile_b&Bottle.COLOR_MASK) == color_b){
                        score+=0.5; 
                    }else if(tile_b!=Bottle.EMPTY_TILE){
                        score-=0.5; 
        ...
        if(place_high){
            score += ((double)(Bottle.HEIGHT-row))/10; 
        }else{
            score+=((double)row)/10; 
        }
    ...
    private double scorePlacementHor(Playfield playfield, int row, int col, int orientation){
        ...
        if(orientation%2==0){
            ...
            int match_counter_t = 0; 
            boolean seq_has_virus_t = false; 
            for(int i=(col-1); i>=0; i--){
            ...
            //only reward complete horizontal clears for the top vitamin
            if(match_counter_t>=3){
                score++; 
                if(seq_has_virus_t){
                    score++; 
                }
            ...
            //reward bottom horizontal matches 
            ...
            // punish severely 
            if(row==1){
                if(match_counter_b<3 && match_counter_t<3){
                    score-=10; 
                }
            }
