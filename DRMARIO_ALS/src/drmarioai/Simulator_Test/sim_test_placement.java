...
import drmarioai.Greedy_BST.*;

import java.util.ArrayList;

import static drmarioai.Bottle.*;

public class sim_test_placement {
    ...
    private int scorePlacementVer(Playfield playfield, int row, int col, int orientation){
        ...
        if(orientation%2==0){
            ...
            if(row>0 && (playfield.playfield[row-1][col]&COLOR_MASK)==color_l){
                score+=5;
                match_left = true; 
                if(playfield.col_has_virus[col]){
                    score+=10; 
            ...
            if(row>0 && (playfield.playfield[row-1][col+1]&COLOR_MASK)==color_r){
                score+=5;
                match_right = true; 
            ...
            boolean match_top = false; 
            if(row>0 && (playfield.playfield[row-1][col]&COLOR_MASK)==color_t){
                score+=5;
                if(playfield.col_has_virus[col]){
                    score+=10; 
                }
            ...
            int color_b = playfield.playfield[row][col] & COLOR_MASK; 
            // check under the vitamin 
            if(row<(HEIGHT-1) && (playfield.playfield[row+1][col]&COLOR_MASK)==color_b){
                score+=5;
                if(playfield.col_has_virus[col]){
                    score+=10; 
                }
            ...
            if(!is_match && playfield.col_has_virus[col]){
                score-=10; 
            }
        ...
        if(orientation%2==0){
            ...
            if(match_counter_b<3 || match_counter_2>=3){
                ...
                if(match_counter_2==0){
                    ...
                    score+=10;
                    // if(playfield.row_has_virus[row-1]){
                    //     score+=5; 
                    // }
                }
            }else{
            ...
            //check left of the bottom vitamin 
            if(col>0 && (playfield.playfield[row][col-1]&COLOR_MASK)==color_b){
                score+=10;
                match_l = true; 
            }
            ...
            //check right of the bottom vitamin 
            if(col<(WIDTH-1) && (playfield.playfield[row][col+1]&COLOR_MASK)==color_b){
                score+=10;
                match_r = true; 
            }
