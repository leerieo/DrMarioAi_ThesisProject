...
public class GreedyV3 {
    ...
    public void processChecks(int pill_l, int pill_r, ItemColumn[] top_items){
        ...
        if(pill_l == pill_r){
            ...
            idx_orientation = TVO_Utils.Horizontal_Lowest_Perfect_2_Match(pill_l, pill_r, top_items); 
            if(idx_orientation[0]>=0){
                col_idx = idx_orientation[0];
                orientation = idx_orientation[1]; 
                return; 
            ...
            idx_orientation = TVO_Utils.Horizontal_Lowest_Half_2_Match(pill_l, pill_r, top_items);
            if(idx_orientation[0]>=0){
                col_idx = idx_orientation[0];
                orientation = idx_orientation[1]; 
                return; 
